...
/******************************************************************************
AddRemoveFilter - Adds or removes a filter
*******************************************************************************/
...
bool PacketFilter::ParseIPAddrStringEx(char* szIpAddr, UINT nStrLen, BYTE* pbHostOrdr, UINT nByteLen, ULONG& uHexAddr)
{

	struct sockaddr_in sa;
	inet_pton(AF_INET, szIpAddr, &(sa.sin_addr));

	// Hack da hack!!
	uint32_t clientIpAddr = ntohl(sa.sin_addr.s_addr);
	uHexAddr = clientIpAddr;

	return false;
}

/******************************************************************************
  AddToBlockList - Add ip addresses to be blocked.
*******************************************************************************/
void PacketFilter::AddToBlockList( char* szIpAddrToBlock )
{
    try
    {
        if( NULL != szIpAddrToBlock )
        {
            ...
            // overriden, only in out params are relevant.
            ParseIPAddrStringEx(szIpAddrToBlock,
                               0,
...
/******************************************************************************
StartFirewall - Start prepared rules for Filter Engine 
*******************************************************************************/
